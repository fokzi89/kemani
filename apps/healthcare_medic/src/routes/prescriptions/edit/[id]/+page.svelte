	...
	<div class="max-w-4xl mx-auto space-y-6">
		...
		{:else}
			...
			<div class="flex border-b border-gray-200 bg-white rounded-t-2xl px-6 pt-2">
				<button onclick={() => activeTab = 'details'} class="flex items-center gap-2 px-6 py-4 text-sm font-bold border-b-2 transition-all {activeTab === 'details' ? 'border-primary-700 text-primary-700' : 'border-transparent text-gray-500 hover:text-gray-700'}">
					<UserPlus class="h-4 w-4" /> Details
				</button>
				<button onclick={() => activeTab = 'drugs'} class="flex items-center gap-2 px-6 py-4 text-sm font-bold border-b-2 transition-all {activeTab === 'drugs' ? 'border-primary-700 text-primary-700' : 'border-transparent text-gray-500 hover:text-gray-700'}">
				...
				{:else if activeTab === 'drugs'}
					<div class="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
						<div class="flex items-center justify-between">
							...
							<button onclick={() => openDrugModal(null)} class="px-4 py-2 bg-primary-100 text-primary-900 hover:bg-primary-200 font-bold rounded-xl flex items-center gap-2 text-sm"><Plus class="h-4 w-4" /> Add</button>
						</div>
						<div class="max-h-[500px] overflow-y-auto pr-2 space-y-4 custom-scrollbar">
							{#each drugs as drug, index}
								<div class="flex items-center justify-between p-5 bg-white border border-gray-200 rounded-2xl shadow-sm hover:shadow-md transition-all gap-4">
									<div class="flex-1">
										<p class="font-bold text-gray-900">{drug.name} <span class="text-xs text-primary-700 font-medium">{drug.dispense_quantity || ''} {drug.dispense_as || ''}</span></p>
										<div class="text-sm text-gray-500 mt-1">{drug.dosage || ''} - {drug.frequency || ''} - {drug.duration || ''}</div>
									</div>
									<div class="flex gap-2">
										<button onclick={() => openDrugModal(index)} class="p-2 text-blue-600 hover:bg-blue-50 rounded-full"><Edit class="h-4 w-4" /></button>
										<button onclick={() => deleteDrug(index)} class="p-2 text-red-600 hover:bg-red-50 rounded-full"><Trash2 class="h-4 w-4" /></button>
									</div>
								</div>
							{/each}
						</div>
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/50 backdrop-blur-sm animate-in fade-in">
		<div class="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
			<div class="sticky top-0 bg-white border-b px-8 py-6 flex items-center justify-between z-10">
				<h3 class="text-xl font-bold">{editingDrugIndex !== null ? 'Edit Drug' : 'Add Drug'}</h3>
