<script lang="ts">
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      alert(`Booking Confirmed for ${firstSlot} on ${selectedDate} (${selectedTimes.length} slot(s))!`);
...
<style>
  ...
  /* Sidebar */
  .booking-widget { background: var(--surface-container-lowest); border: 1px solid var(--outline-variant); padding: 1.25rem; border-radius: 1rem; position: sticky; top: 100px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.05); }
  ...
  .dates-row { display: flex; gap: 0.75rem; overflow-x: auto; padding-bottom: 0.75rem; margin-bottom: 1.25rem; }
