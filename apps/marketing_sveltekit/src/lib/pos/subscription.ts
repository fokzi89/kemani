...
import { Subscription, SubscriptionPlanTier, SubscriptionStatus } from '@/lib/types/database';
...
export const PLANS: Record<SubscriptionPlanTier, PlanLimits> = {
    ...
    enterprise: {
        ...
        transactionFee: 50, // Negotiable usually, but standard for now
        maxBranches: 9999,
        maxStaff: 9999,
        maxProducts: 999999,
        features: {
...
export class SubscriptionService {
    ...
    /**
     * Get tenant's current subscription. Defaults to FREE if none found.
     */
    static async getCurrentSubscription(tenantId: string): Promise<Subscription> {
        const supabase = await createClient();
        const { data } = await supabase
            .from('subscriptions') // Assumes table exists
            .select('*')
    ...
    static async checkFeatureAccess(tenantId: string, feature: keyof PlanLimits['features']): Promise<boolean> {
        ...
        if (feature === 'canUseAi') {
            // Available if plan allows it OR if add-on is enabled
            return plan.features.canUseAi || (sub.ai_addon_enabled === true);
        }
    ...
    static async toggleAiAddon(tenantId: string, enabled: boolean) {
        const supabase = await createClient();
        const { error } = await supabase
            .from('subscriptions')
            .update({
