		...
		try {
			...
			const { data: sent, error: msgError } = await supabase
				...
				.insert({
					...
					message_type: uploadType === 'image' ? 'image' : 'image', // 'file' not in enum, using image as fallback
					media_url: publicUrl,
					metadata: { 
...
{#if showProductModal}
	<ProductSelectionModal {tenantId} onSelect={handleProductSelect} onClose={() => showProductModal = false} />
{/if}
