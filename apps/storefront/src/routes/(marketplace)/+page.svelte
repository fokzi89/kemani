<script lang="ts">
	...
	// Tenant data is loaded server-side in +page.server.ts
	// Slug is already resolved and validated — page receives clean data
	...
	$: storefront = {
		...data.tenant,
		banner_url: data.tenant.banner_url
			|| 'https://images.unsplash.com/photo-1576091160550-217359f51f8c?q=80&w=2000&auto=format&fit=crop'
	};
...
<svelte:head>
	<title>{storefront.name} — Shop Online</title>
	<meta name="description" content="Shop {storefront.name} online — quality products, fast delivery." />
</svelte:head>

<!-- Root div — CSS vars injected from tenant brand_color -->
<div class="min-h-screen bg-[#F8FAFC]" style="--brand:{brandColor}; --brand-light:{brandColorLight};">

	<!-- Tenant URL strip -->
	<div class="brand-strip py-2 text-center text-[11px] font-black tracking-widest uppercase flex items-center justify-center gap-2">
		<Globe class="h-3 w-3 opacity-70" />
		<span class="opacity-70">Your store:</span>
		<span>{storeUrl}</span>
	</div>
			...
			<div class="flex justify-between items-center h-20">
				...
				<a href="/" class="flex items-center gap-3">
					{#if storefront.logo_url}
						<img src={storefront.logo_url} alt="{storefront.name} logo" class="h-10 w-10 rounded-xl object-cover shadow-md ring-2" style="--tw-ring-color:{brandColor}22;" />
					{:else}
						<div class="h-10 w-10 rounded-xl brand-bg flex items-center justify-center text-white font-black text-lg shadow-md">
							{storefront.name.charAt(0)}
						</div>
					{/if}
					<div class="flex flex-col leading-tight">
						<span class="text-lg font-black text-gray-900 tracking-tight">{storefront.name}</span>
						<span class="text-[10px] font-bold text-gray-400 tracking-widest">{storeUrl}</span>
					</div>
				</a>

				<!-- Actions -->
				<div class="flex items-center gap-3">
					<a href="/medics" class="hidden sm:flex group items-center gap-2 h-11 px-5 rounded-2xl border border-gray-100 bg-white transition-all duration-200 hover:bg-gray-50 text-brand font-bold text-sm" style="color:{brandColor};">
						<ShieldCheck class="w-5 h-5 opacity-80" />
						Medics
					</a>
					<a href="/cart" class="group relative flex items-center gap-2 h-11 px-5 rounded-2xl border border-gray-100 bg-gray-50 transition-all duration-200 hover:brand-bg">
						<ShoppingCart class="w-5 h-5 text-gray-500 group-hover:text-white transition-colors" />
						<span class="text-sm font-bold text-gray-700 group-hover:text-white transition-colors">Cart</span>
						{#if cartItemCount > 0}
							<span class="absolute -top-1.5 -right-1.5 brand-bg text-white text-[9px] font-black rounded-full h-5 w-5 flex items-center justify-center border-2 border-white">{cartItemCount}</span>
						{/if}
					</a>
					<a href="/auth/login" class="h-11 w-11 bg-gray-900 rounded-2xl flex items-center justify-center text-white hover:scale-105 transition-transform active:scale-95">
		...
		<div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
			<div class="max-w-xl space-y-4">
				...
				<p class="text-sm text-gray-300 font-medium max-w-md">
					{storefront.description || 'Browse our full catalog — available for delivery or pickup.'}
				</p>
			</div>
		</div>
	</section>
		...
		<div class="grid lg:grid-cols-4 gap-8">
			...
			<aside class="hidden lg:block space-y-6">
				...
				<div class="p-5 bg-white rounded-2xl border border-gray-100 shadow-sm space-y-5">
					<div>
						...
						<button onclick={() => loadProducts()} class="w-full mt-3 py-2 text-white text-xs font-black rounded-lg uppercase tracking-widest brand-bg">Apply</button>
					</div>
					<label class="flex items-center gap-3 cursor-pointer">
						<input type="checkbox" bind:checked={inStockOnly} onchange={() => loadProducts()} class="w-5 h-5 rounded-lg border-gray-200" />
			...
			<main class="lg:col-span-3 space-y-6">
				...
				<div class="flex flex-col md:flex-row gap-3">
					...
					<div class="flex gap-2">
						<select bind:value={sortBy} onchange={() => loadProducts()} class="h-14 px-4 bg-white border border-gray-100 rounded-2xl shadow-sm text-sm font-bold text-gray-700 outline-none cursor-pointer">
						...
						<div class="flex p-1 bg-white border border-gray-100 rounded-2xl shadow-sm">
							<button onclick={() => viewMode = 'grid'} class="p-3 rounded-xl transition-all" style={viewMode==='grid'?`background:${brandColorLight};color:${brandColor};`:'color:#9ca3af;'}><LayoutGrid class="h-5 w-5" /></button>
							<button onclick={() => viewMode = 'list'} class="p-3 rounded-xl transition-all" style={viewMode==='list'?`background:${brandColorLight};color:${brandColor};`:'color:#9ca3af;'}><List class="h-5 w-5" /></button>
						</div>
					</div>
				</div>
				...
				{:else}
							...
							<div class="group bg-white rounded-3xl border border-gray-100 p-4 transition-all duration-300 hover:shadow-xl flex {viewMode==='list'?'flex-row gap-5':'flex-col'}">
								...
								<div class="flex-1 flex flex-col">
									...
									<div class="flex items-center justify-between">
										...
										{#if product.is_available && product.stock_quantity > 0}
											<button onclick={() => addToCart(product)} class="h-11 w-11 brand-bg text-white rounded-2xl flex items-center justify-center shadow-lg hover:scale-105 active:scale-90 transition-all">
												<Plus class="h-5 w-5" />
											</button>
										{:else}
					...
					{#if totalPages > 1}
						<div class="flex items-center justify-center gap-2 pt-4">
							<button onclick={() => handlePageChange(currentPage-1)} disabled={currentPage===1} class="p-3 rounded-xl bg-white border border-gray-100 disabled:opacity-30"><ArrowLeft class="h-5 w-5" /></button>
							{#each Array(totalPages) as _, i}
								<button onclick={() => handlePageChange(i+1)} class="h-11 w-11 rounded-xl text-sm font-bold transition-all" style={currentPage===i+1?`background:${brandColor};color:white;`:'background:white;color:#6b7280;'}>{i+1}</button>
							{/each}
							<button onclick={() => handlePageChange(currentPage+1)} disabled={currentPage===totalPages} class="p-3 rounded-xl bg-white border border-gray-100 disabled:opacity-30"><ChevronRight class="h-5 w-5" /></button>
						</div>
					{/if}
				{/if}
	...
	<!-- Footer -->
	<footer class="bg-gray-950 text-white pt-14 pb-8 mt-10">
		<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
			<div class="grid grid-cols-1 md:grid-cols-3 gap-10 pb-10 border-b border-white/5">
				<div class="space-y-4 md:col-span-1">
					<div class="flex items-center gap-3">
						{#if storefront.logo_url}
							<img src={storefront.logo_url} alt="logo" class="h-10 w-10 rounded-xl object-cover" />
						{:else}
							<div class="h-10 w-10 rounded-xl brand-bg flex items-center justify-center text-white font-black text-lg">{storefront.name.charAt(0)}</div>
						{/if}
						<div>
							<p class="text-base font-black">{storefront.name}</p>
							<p class="text-[10px] font-bold" style="color:{brandColor};">{storeUrl}</p>
						</div>
					</div>
					<p class="text-gray-400 text-sm leading-relaxed">{storefront.description || 'Shop online with confidence.'}</p>
				</div>
				<div>
					<h4 class="text-xs font-black uppercase tracking-widest mb-5" style="color:{brandColor};">Quick Links</h4>
					<ul class="space-y-3 text-sm text-gray-400 font-bold">
						<li><a href="/" class="hover:text-white transition-colors">Storefront</a></li>
				...
				<div>
					<h4 class="text-xs font-black uppercase tracking-widest mb-5" style="color:{brandColor};">Newsletter</h4>
					<p class="text-gray-400 text-xs mb-4">Stay updated with {storefront.name} deals.</p>
					<div class="flex gap-2">
						<input type="email" placeholder="your@email.com" class="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2 text-sm outline-none" />
						<button class="brand-bg hover:opacity-90 p-2.5 rounded-xl transition-opacity"><ArrowRight class="h-4 w-4" /></button>
					</div>
				</div>
			</div>
			<div class="pt-7 flex flex-col md:flex-row justify-between items-center gap-3 text-[10px] font-bold text-gray-600 uppercase tracking-widest">
...
<style>
	...
	.brand-strip { background-color: var(--brand, #4f46e5); color: white; }
