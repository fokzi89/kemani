...
$migrationsDir = "c:\Users\afokeoghene.akpomiem\kemani\supabase\migrations"
$outputFile = "c:\Users\afokeoghene.akpomiem\kemani\supabase\combined_migration.sql"

# Check if migrations directory exists
if (-not (Test-Path $migrationsDir)) {
    Write-Host "x Error: Migrations directory not found: $migrationsDir" -ForegroundColor Red
    exit 1
}
...
if ($migrationFiles.Count -eq 0) {
    Write-Host "x Error: No migration files found in $migrationsDir" -ForegroundColor Red
    exit 1
}
...
Write-Host ""
Write-Host "Alternative: Apply migrations individually in order" -ForegroundColor Yellow
Write-Host ""
Write-Host "==================================================" -ForegroundColor Cyan
...
if (Test-Path $outputFile) {
    ...
    # Start-Process $outputFile # Disabled in agent environment
}
